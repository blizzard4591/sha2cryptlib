/*
	...
	MODIFIED CODE:
	- Philipp Berger, 01.2015 (multi-platform comparability, re-create buffer on every call)
	*/
...
# define UNALIGNED_P(p) (((uintptr_t) p) % sizeof (uint32_t) != 0)
	...
	char *
		sha256_crypt_r(const char *key, const char *salt, char *buffer, size_t buflen) {
#if defined(_WIN32) || defined(_WIN64) || defined(_MSC_VER)
		__declspec(align(__ALIGNOF_UINT32_T)) unsigned char alt_result[32];
		__declspec(align(__ALIGNOF_UINT32_T)) unsigned char temp_result[32];
#else
		...
		if (rounds_custom) {
#if defined(_WIN32) || defined(_WIN64) || defined(_MSC_VER)
			// %zu is not supported. Use %Iu
			int n = snprintf(cp, MAX(0, buflen), "%s%Iu$",
		...
		buflen -= (uint32_t)MIN((size_t)MAX(0, buflen), salt_len);
...
#define b64_from_24bit(B2, B1, B0, N)					      \
  do {									      \
	...
	*/
	char *
		sha256_crypt_s(const char *key, const char *salt) {
		/* We don't want to have an arbitrary limit in the size of the
		password.  We can compute an upper bound for the size of the
		result in advance and so we can prepare the buffer we pass to
		`sha256_crypt_r'.  */
