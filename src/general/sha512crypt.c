...
char *
sha512_crypt_r (const char *key, const char *salt, char *buffer, size_t buflen)
{
#if defined(_WIN32) || defined(_WIN64) || defined(_MSC_VER)
	__declspec(align(__ALIGNOF_UINT64_T)) unsigned char alt_result[64];
	__declspec(align(__ALIGNOF_UINT64_T)) unsigned char temp_result[64];
#else
...
*/
char* sha512_crypt_s(const char *key, const char *salt) {
	/* We don't want to have an arbitrary limit in the size of the
	password.  We can compute an upper bound for the size of the
	result in advance and so we can prepare the buffer we pass to
	`sha512_crypt_r'.  */
